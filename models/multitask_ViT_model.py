...
class BinModel(nn.Module):
    ...
    def forward(self, img):
        ...
        # reshape
        # being consistent with the input parameters of function "build_model"
        # p1 = patch_size, p2 = patch_size,  h=image_size[0]//patch_size)
        rec_images = rearrange(pred_pixel_values, 'b (h w) (p1 p2 c) -> b c (h p1) (w p2)',
                                 p1 = 16, p2 = 16,  h=256//16)

        return rec_images, patches
...
class multitask_ViT_model(BaseModel):
    def __init__(self, opt):
        super(multitask_ViT_model, self).__init__(opt)
        ...
        self.netG = build_model('base',256,16).to(self.device)
        if opt['dist']:

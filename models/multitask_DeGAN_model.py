...
class DEGAN(nn.Module):
    def __init__(self):
        ...
        self.upv6 = nn.ConvTranspose2d(1024, 512, 2, stride=2)
        ...
        self.upv7 = nn.ConvTranspose2d(512, 256, 2, stride=2)
        ...
        self.upv8 = nn.ConvTranspose2d(256, 128, 2, stride=2)
        ...
        self.upv9 = nn.ConvTranspose2d(128, 64, 2, stride=2)
        ...
        self.conv10_1 = nn.Conv2d(64, 3, kernel_size=1, stride=1)
...
class multitask_DEGAN_model(BaseModel):
    def __init__(self, opt):
        super(multitask_DEGAN_model, self).__init__(opt)
        ...
        self.netG = DEGAN().to(self.device)
        if opt['dist']:
...
###############################   Distilation setting
        if train_opt['distill']:
            if opt['name']=='DEGAN':
                self.netG_Pre = DEGAN().to(self.device)
            else :
...
####################################################################
    ...
    def optimize_parameters(self, step):
        ...
        if self.opt['train']['distill']:
            ...
            with torch.no_grad():
                self.netG_Pre.eval()
                gtinstf,gtfusef = self.netG_Pre(var_fake.detach())
